...
func loadSystemRoots() (*CertPool, error) {
	...
	for i := range rootCerts {
		rc := &rootCerts[i]
		p.addCertFunc(rc.s224, rc.rawSubj, certReader(rc.cert))
	}
	return p, nil
}

func certReader(s string) func() (*Certificate, error) {
	var once sync.Once
	var c *Certificate
	var err error
	return func() (*Certificate, error) {
		once.Do(func() {
			h := (*unsafeheader.String)(unsafe.Pointer(&s))
			buf := unsafe.Slice((*byte)(h.Data), len(s))
			c, err = ParseCertificate(buf)
		})
		return c, err
	}
}

var rootCerts = [...]struct {
	s224          sum224
	rawSubj, cert string
}{
`
