...
func TestAffine(t *testing.T) {
	...
	t.Run("Addition", func(t *testing.T) {
		for i := 0; i < testTimes; i++ {
			...
			if gotX.Cmp(wantX) != 0 {
				ReportError(t, gotX, wantX, K1, K2)
			}
			if gotY.Cmp(wantY) != 0 {
...
type IsAtInfinityIer interface {
	elliptic.Curve
...
func TestCombinedMult(t *testing.T) {
	...
	for i := 0; i < testTimes; i++ {
		...
		if gotX.Cmp(wantX) != 0 {
			ReportError(t, gotX, wantX, K, K1, K2)
		}
		if gotY.Cmp(wantY) != 0 {
			ReportError(t, gotY, wantY)
		}
