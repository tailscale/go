...
func TestPointDouble(t *testing.T) {
	...
	t.Run("2P=P+P", func(t *testing.T) {
		...
		for i := 0; i < 128; i++ {
			...
			if gotX.Cmp(wantX) != 0 {
				ReportError(t, gotX, wantX, P)
			}
			if gotY.Cmp(wantY) != 0 {
				ReportError(t, gotY, wantY)
			}
		}
	})
}
...
func TestPointAdd(t *testing.T) {
	...
	t.Run("P+Q=R", func(t *testing.T) {
		for i := 0; i < 128; i++ {
			...
			if gotX.Cmp(wantX) != 0 {
				ReportError(t, gotX, wantX, P, Q)
			}
			if gotY.Cmp(wantY) != 0 {
				ReportError(t, gotY, wantY, P, Q)
			}
		}
	})
}
...
func TestPointMixAdd(t *testing.T) {
	...
	t.Run("P+∞=P", func(t *testing.T) {
		...
		if gotX.Cmp(wantX) != 0 {
			ReportError(t, gotX, wantX, jP)
		}
		if gotY.Cmp(wantY) != 0 {
			ReportError(t, gotY, wantY)
		}
		if gotZ.Cmp(wantZ) != 0 {
			ReportError(t, gotZ, wantZ)
		}
	})
	...
	t.Run("P+Q=R", func(t *testing.T) {
		for i := 0; i < 128; i++ {
			...
			if gotX.Cmp(wantX) != 0 {
				ReportError(t, gotX, wantX, jP, aP)
			}
			if gotY.Cmp(wantY) != 0 {
				ReportError(t, gotY, wantY)
			}
