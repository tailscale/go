...
var (
	stdout    = flag.Bool("stdout", false, "write to stdout instead of sizeclasses.go")
	filename  = flag.String("out", "sizeclasses.go", "write to file `filename`")
	buildtags = flag.String("tags", "", "add build tags `tags` to file")
	pageshift = flag.Int("pageshift", 13, "set _PageShift to `pageshift`")
	maxwaste  = flag.Int("maxwaste", 8, "set max waste factor to 1+1/`maxwaste`")
	nclasses  = flag.Int("nclasses", 68, "number of size classes anticipated, set to 0 to disable check")
	maxsmall  = flag.Int("maxsmall", 32<<10, "max 'small' allocation size")
)
...
var (
	// Vars that we use and will transfer to the runtime.
	maxSmallSize int
	pageShift    int
...
func makeClasses() []class {
	...
	for size := align; size <= maxSmallSize; size += align {
		...
		allocsize := pageSize
		for allocsize%size > allocsize / *maxwaste {
			allocsize += pageSize
		}
	...
	if *nclasses != 0 && len(classes) != *nclasses {
		fmt.Printf("requested %d classes, got %d\n", *nclasses, len(classes))
